...
impl Manager {
    ...
    fn pddb_exists(&self, dict: &str, key_name: &str, basis: Option<String>) -> bool {
        match self.pddb.get(dict, &key_name, basis.as_deref(), false, false, None, Some(vault_lite::basis_change))
        {
            Ok(_) => return true,
    ...
    fn pddb_get(&self, dict: &str, key_name: &str) -> Result<Vec<u8>, Error> {
        match self.pddb.get(dict, key_name, None, false, false, None, Some(vault_lite::basis_change)) {
            Ok(mut record) => {
